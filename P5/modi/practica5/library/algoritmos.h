...
template <class Key>
void quickSort(staticSequence<Key>& data, int size, bool trace, int& comparaciones) {
  Qsort(data, 0, data.getSize() - 1, trace, comparaciones);
}

template <class Key>
void Qsort(staticSequence<Key>& data, int inicio, int fin, bool trace, int& comparaciones) {
  int i = inicio;
  ...
  if (inicio < f) {
    //std::cout << data << std::endl;
    Qsort(data, inicio, f, trace, comparaciones);
  }
  if (i < fin) {
    //std::cout << data << std::endl;
    Qsort(data, i, fin, trace, comparaciones);
  }
}
...
template <class Key>
void shellSort(staticSequence<Key>& data, int n, bool trace, int& comparaciones) {
  int delta = n ;
  double alpha = 0.7;
  while (delta > 1) {
    delta = static_cast<int>(delta * alpha);
    deltasort(delta,data, n, trace, comparaciones);
  }
} 

template <class Key>
void deltasort(int delta, staticSequence<Key>& sec, int n, bool trace, int& comparaciones) {
