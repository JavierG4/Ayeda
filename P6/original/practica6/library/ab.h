...
class ABB : public AB<Key>{
 public:
   ...
   void DeleteNode(NodoB<Key>* node);
   bool Buscar(const Key&);
   NodoB<Key>* BuscarRama(NodoB<Key>* nodo, const Key& clave_dada);
   bool Insertar(const Key&);
...
bool ABB<Key>::Buscar(const Key& clave_dada) {
  bool resultado = BuscarRama(raiz_, clave_dada) != nullptr;
  return resultado;
}

template <class Key>
NodoB<Key>* ABB<Key>::BuscarRama(NodoB<Key>* nodo, const Key& clave_dada) {
  if (nodo == nullptr) {
    return nullptr;
  }
  ...
  if (clave_dada < nodo->GetDato()) {
    return BuscarRama(nodo->GetIzquierda(), clave_dada );
  }
  return BuscarRama(nodo->GetDerecha(), clave_dada );
}
