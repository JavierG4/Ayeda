...
Latice::Latice(int n, int frontera, std::string fichero) {
  ...
  if (fichero == " ") {
    if (frontera == 0 || frontera == 1) {
      ...
      numero_celulas_ = n + 4;
      latice_.resize(n + 4);
      for ( int i = 2; i < numero_celulas_ -2; i++) { 
        if (n/2 == i) {
      ...
      if (frontera == 0) {
        ...
        latice_[1] = new Celula(Posicion(1), Estado(0));
        latice_[n + 2] = new Celula(Posicion(n + 2), Estado(0));
        std::cout << "entra2" << std::endl;
        std::cout << numero_celulas_ << std::endl;
      } else {
        latice_[0] = new Celula(Posicion(0), Estado(1));
        latice_[n + 1] = new Celula(Posicion(n + 1), Estado(1));
        latice_[1] = new Celula(Posicion(1), Estado(1));
        latice_[n + 2] = new Celula(Posicion(n + 2), Estado(1));
      }
    } else {
      numero_celulas_ = n;
      latice_.resize(n);
